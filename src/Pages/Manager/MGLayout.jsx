...
const ICONS = {
  ...
  newdoc:    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>,
...
const NAV_MAIN = [
  ...
  { key: "newdoc",    label: "New Document", icon: "newdoc" },
  { key: "documents", label: "Documents",    icon: "docs" },
  { key: "approvals", label: "Approvals",    icon: "approvals", badge: 1 },
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[220px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-2 px-[14px] py-[10px] border-b border-white/10">
          <div className="w-7 h-7 rounded-full bg-amber-700 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">MG</div>
          <div>
            <div className="text-white/90 text-[11px] font-semibold">{user?.u || "manager"}</div>
            <div className="text-white/40 text-[10px]">Manager</div>
          </div>
        </div>
...
function Topbar({ title, onMenuClick, onLogout }) {
  ...
  return (
    <header className="h-[46px] bg-white dark:bg-[#1A2433] border-b border-slate-200 dark:border-slate-700 flex items-center px-4 gap-3 flex-shrink-0 sticky top-0 z-20 transition-colors">
      ...
      <span className="bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-[11px] font-bold px-[10px] py-[2px] rounded-full hidden sm:inline">Manager</span>
...
export function MGLayout({ activePage, onNavigate, onLogout, title, children, user }) {
  const [mobileOpen, setMobileOpen] = useState(false);
  return (
    <div className="flex h-screen bg-slate-100 dark:bg-[#0F1623] transition-colors overflow-hidden">
      <Sidebar activePage={activePage} onNavigate={onNavigate} onLogout={onLogout}
        mobileOpen={mobileOpen} onMobileClose={() => setMobileOpen(false)} user={user} />
