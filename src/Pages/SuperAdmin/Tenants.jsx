...
function RegisterModal({ onClose, onSaved }) {
  ...
  async function handleStep1() {
    ...
    try {
      const res  = await fetch(`${API}/tenants`, {
        method:  "POST",
        headers: authHeaders(),
        body:    JSON.stringify({ tenantName: tenantName.trim(), tenantCode: tenantCode.trim().toUpperCase() }),
      });
  ...
  async function handleStep2() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success) return setError(data.message || "Failed to create admin.");
      setDone(true);
      onSaved();
    } catch {
      setError("Server error. Try again.");
    ...
    <div className="space-y-4">
      ...
      {loading && (
        <div className="flex items-center justify-center py-16 gap-3 text-slate-400">
          <svg className="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M12 2a10 10 0 0 1 10 10"/></svg>
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-400">
          ...
          {!search && <button onClick={() => setShowModal(true)} className="text-[12px] text-blue-600 dark:text-blue-400 font-semibold hover:underline">+ Register first tenant</button>}
        </div>
      )}
        ...
        <div className="space-y-3">
          {filtered.map(t => (
            <div key={t.id} className="bg-white dark:bg-[#1A2433] border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden transition-all hover:shadow-md">
              <div className="flex items-center gap-3 px-4 py-4 cursor-pointer" onClick={() => setExpanded(expanded === t.id ? null : t.id)}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-bold text-slate-800 dark:text-slate-100 truncate">{t.tenantCode}_{t.tenantName}</div>
                  <div className="text-[10px] text-slate-400 flex items-center gap-2 mt-px">
                    <span>ID: {t.id}</span><span>·</span>
