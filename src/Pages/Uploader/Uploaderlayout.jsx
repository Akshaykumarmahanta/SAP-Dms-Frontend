...
const ICONS = {
  ...
  upload:    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0018 9h-1.26A8 8 0 103 16.3"/></svg>,
...
const NAV_ITEMS = [
  ...
  { key: "documents", label: "My Documents",    icon: "docs" },
];
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[220px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-2 px-[14px] py-[10px] border-b border-white/10">
          <div className="w-7 h-7 rounded-full bg-purple-700 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">
            UP
          </div>
          <div>
            <div className="text-white/90 text-[11px] font-semibold">{user?.u || "uploader"}</div>
            <div className="text-white/40 text-[10px]">Uploader</div>
          </div>
        </div>
...
function Topbar({ title, onMenuClick, onLogout }) {
  ...
  return (
    <header className="h-[46px] bg-white dark:bg-[#1A2433] border-b border-slate-200 dark:border-slate-700 flex items-center px-4 gap-3 flex-shrink-0 sticky top-0 z-20 transition-colors">
      ...
      <span className="bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 text-[11px] font-bold px-[10px] py-[2px] rounded-full hidden sm:inline">
        Uploader
      </span>
...
export function UploaderLayout({ activePage, onNavigate, onLogout, title, children, user }) {
