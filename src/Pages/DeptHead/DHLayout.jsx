...
const NAV_MAIN = [
  ...
  { key: "documents", label: "Dept Documents", icon: "docs" },
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[220px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-2 px-[14px] py-[10px] border-b border-white/10">
          <div className="w-7 h-7 rounded-full bg-orange-600 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">DH</div>
          <div>
            <div className="text-white/90 text-[11px] font-semibold">{user?.u || "depthead"}</div>
            <div className="text-white/40 text-[10px]">DeptHead</div>
          </div>
        </div>
...
export function DHLayout({ activePage, onNavigate, onLogout, title, children, user }) {
