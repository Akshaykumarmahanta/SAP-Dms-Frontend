...
const NAV_ITEMS = [
  ...
  { key: "auditlog",  label: "Full Audit Log", icon: "audit" },
];
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[220px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-2 px-[14px] py-[10px] border-b border-white/10">
          <div className="w-7 h-7 rounded-full bg-orange-600 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">
            AU
          </div>
          <div>
            <div className="text-white/90 text-[11px] font-semibold">{user?.u || "auditor"}</div>
            <div className="text-white/40 text-[10px]">Auditor</div>
          </div>
        </div>
...
function Topbar({ title, onMenuClick, onLogout }) {
  ...
  return (
    <header className="h-[46px] bg-white dark:bg-[#1A2433] border-b border-slate-200 dark:border-slate-700 flex items-center px-4 gap-3 flex-shrink-0 sticky top-0 z-20 transition-colors">
      ...
      <span className="bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 text-[11px] font-bold px-[10px] py-[2px] rounded-full hidden sm:inline">
        Auditor
      </span>
...
export function AuditorLayout({ activePage, onNavigate, onLogout, title, children, user }) {
